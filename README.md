...
1. **Initial Setup and Inspection**
   ...
   * `dataset\_set.py`: Configure paths and perform an initial inspection of raw dataset files.
...
3. **Phase 2: Feature Engineering \& Representation Learning**
   ...
   * `Single Unified Feature Space (Harmonized Meta-Schema).py`: Maps all datasets to the common 32-feature schema, creating the `H\_\*.parquet` files for cross-domain experiments.
   * `Temporal transformation of tabular intrusion detection data into sequence data.py`: Creates sequence data using a sliding window approach, saving `seq\_\*.npy` files.
   * `Graph Representation Learning pipeline for intrusion detection data.py`: Creates graph embeddings using Node2Vec, saving `graph\_\*.npy` files.
...
* **Hyperparameters:** `n\_estimators`, `max\_depth`, `learning\_rate`.
